...
inv1 = (1-lambda(f1))./b(f1).*sqrt((nu(f1)-2)./nu(f1)).*tinv(u(f1)./(1-lambda(f1)),nu(f1))-a(f1)./b(f1);    %DHO replace tdis_inv with tinv
inv2 = (1+lambda(f2))./b(f2).*sqrt((nu(f2)-2)./nu(f2)).*tinv(0.5+1./(1+lambda(f2)).*(u(f2)-(1-lambda(f2))./2),nu(f2))-a(f2)./b(f2); %DHO replace tdis_inv with tinv
inv = -999.99*ones(T,1);
inv(f1) = inv1;
inv(f2) = inv2;